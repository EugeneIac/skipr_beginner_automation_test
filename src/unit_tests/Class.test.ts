...
describe('Class unit test', () => {
    ...
    describe('getPosts2', () => {
        it('should fetch post data for /posts/2 successfully', async () => {
            ...
            mock.onGet('/posts/2').reply(200, mockPostsResponse);

            const data = await testClass.getPosts2();
            expect(data).toEqual(mockPostsResponse);
        });

        it('should throw an error if the getPosts2 request fails', async () => {
            mock.onGet('/posts/2').reply(500);
